...
while t<t_final
    ...
    while (error_s>tol_s) || (error_cp>tol_c)
        ...
        M_a_s_wmb = convectionUpwindTerm(-dlabda_w_face.*grad_p0);
        M_a_cs_wmb = convectionUpwindTerm(-k_face.*krw_face.*d_mu_dcs.*grad_p0);
        M_a_cp_wmb = -convectionUpwindTerm(k_face.*krw_face.*d_mu_dcp.*grad_p0);

        % oil mass balance (omb)
        [M_t_s_omb, RHS_t_s_omb] = transientTerm(sw_init, dt, -phi);
        M_d_p_omb = diffusionTerm(-labda_o_face);
        M_a_s_omb = -convectionUpwindTerm((dlabda_o_face.*grad_p0));
        ...
        M_a_s_smb = convectionUpwindTerm(-dlabda_w_face.*cs_face.*grad_p0);        
        M_d_p_smb = diffusionTerm(-labda_w_face.*cs_face);
        M_a_cs_smb = convectionUpwindTerm(-k_face.*krw_face.*d_cs_mu_dcs.*grad_p0);
        M_a_cp_smb = -convectionUpwindTerm(k_face.*krw_face.*d_cs_mu_dcp.*grad_p0);
        ...
        M_a_s_pmb = convectionUpwindTerm(-dlabda_w_face.*cp_face.*grad_p0);
        % M_a_cp, RHS_a_cp = convectionTvdTerm(uw, cp_val, FL, ut)
        M_d_p_pmb = diffusionTerm(-labda_w_face.*cp_face);
        M_a_cs_pmb = convectionUpwindTerm(-k_face.*krw_face.*d_cp_mu_dcs.*grad_p0);
        M_a_cp_pmb = -convectionUpwindTerm(k_face.*krw_face.*d_cp_mu_dcp.*grad_p0);
