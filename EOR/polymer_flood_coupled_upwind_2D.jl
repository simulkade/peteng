...
mu_oil = 10e-3 # Pa.s
perm = 0.1e-12
poros = 0.3*(ones(Nx, Ny)+rand(Nx,Ny)/100)
...
t_final = pv_inj*Lx/(u_inj/poros[1]) # s final time
...
while t<t_final
    ...
    while (error_s>tol_s) || (error_cp>tol_c)
        ...
        cs_new = x_sol[(1:(Nx+2)*(Ny+2))+2*(Nx+2)*(Ny+2)]
        cp_new = x_sol[(1:(Nx+2)*(Ny+2))+3*(Nx+2)*(Ny+2)]

        error_s = sum(abs, reshape(s_new, Nx+2, Ny+2)[2:end-1,2:end-1][:]-internalCells(sw_val)[:])
        error_cp = sum(abs, reshape(cp_new, Nx+2, Ny+2)[2:end-1,2:end-1][:]-internalCells(cp_val)[:])
        ...
        cs_val = createCellVariable(m, internalCells(cs_val), BCcs)
        cp_val = createCellVariable(m, internalCells(cp_val), BCcp)
