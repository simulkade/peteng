...
while t<t_final
    ...
    while (error_s>tol_s) || (error_cp>tol_c)
        ...
        RHS = [RHS_bc_p+RHS_t_s_wmb+M_a_s_wmb*sw_val.value[:]+M_a_cs_wmb*cs_val.value[:]+M_a_cp_wmb*cp_val.value[:];
               ...
               RHS_t_cs_smb+RHS_bc_cs+M_a_s_smb*sw_val.value[:]+M_a_cs_smb*cs_val.value[:]+M_a_cp_smb*cp_val.value[:];
               RHS_t_cp_pmb+RHS_bc_cp+M_a_s_pmb*sw_val.value[:]+M_a_cs_pmb*cs_val.value[:]+M_a_cp_pmb*cp_val.value[:]]
