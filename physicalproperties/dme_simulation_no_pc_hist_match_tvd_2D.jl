...
BCp.left.a[:]  = k_face.xvalue[1,:]/μ_water(c_inj1)
BCp.left.b[:]  = 0.0
BCp.left.c[:]  = -u_inj
...
while t<t_final2
  if t>t_final1 && boundary_switch
    ...
    BCc.left.b[:]  = 1.0
    BCc.left.c[:]  = c_inj2
    BCp.left.a[:]  = k_face.xvalue[1,:]/μ_water(c_inj2)
    BCp.left.b[:]  = 0.0
    BCp.left.c[:]  = -u_inj
  ...
  while error_s>tol_s || error_c>tol_c
    ...
    M_a_s_wmb, RHS_a_s_wmb = convectionTvdTerm(-rho_water*dλ_w_face.*∇p0, sw_val, FL, ut)
    M_a_c_wmb, RHS_a_c_wmb = convectionTvdTerm(
                rho_water*dμ_water_face./mu_water_face.*λ_w_face.*∇p0,
                c_val, FL, ut)
    ...
    # M_a_c_omb = convectionTvdTerm(-dρ_dμ_oil.*k_face.*kro_face.*(∇p0+dpc_face.*∇s0))
    M_a_c_omb, RHS_a_c_omb = convectionTvdTerm(-dρ_dμ_oil.*k_face.*kro_face.*∇p0, c_val, FL, ut)
    M_a_s_omb, RHS_a_s_omb = convectionTvdTerm(-rho_oil_face.*(dλ_o_face.*∇p0), sw_val, FL, ut)
    ...
    M_a_s_dme, RHS_a_s_dme  = convectionTvdTerm(-(rho_water*c_face.*dλ_w_face+
                                    ...
                                    #   (dλ_o_face.*dpc_face+λ_o_face.*d2pc_face).*∇s0)
    M_a_c_dme, RHS_a_c_dme  = convectionTvdTerm(
                                -(rho_water*(mu_water_face-dμ_water_face.*c_face)
    ...
    RHS = [RHS_bc_p+RHS_t_s_wmb+RHS_a_s_wmb+RHS_a_c_wmb+M_a_s_wmb*sw_val.value[:]+M_a_c_wmb*c_val.value[:];
           RHS_bc_s+RHS_t_s_omb+RHS_t_c_omb+RHS_a_c_omb+RHS_a_s_omb+(M_a_s_omb+M_d_s_omb)*sw_val.value[:]+M_a_c_omb*c_val.value[:];
           RHS_bc_c+RHS_t_s_dme+RHS_t_c_dme+RHS_a_c_dme+RHS_a_s_dme+(M_a_s_dme+M_d_s_dme)*sw_val.value[:]+M_a_c_dme*c_val.value[:];]
    ...
    c_new = x_sol[(1:(Nx+2)*(Ny+2))+(2*(Nx+2)*(Ny+2))]
    error_s = sumabs(reshape(s_new, Nx+2, Ny+2)[2:end-1, 2:end-1]-internalCells(sw_val))
    error_c = sumabs(reshape(c_new, Nx+2, Ny+2)[2:end-1, 2:end-1]-internalCells(c_val))
    # println(error_s)
    # println(error_c)
    p_val.value[:] = p_new[:]
    sw_val.value[:] = s_new[:]
