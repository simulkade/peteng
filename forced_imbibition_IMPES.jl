...
"""
function forced_imb_impes(SF0, SW0)
...
f1(sw)=pc_imb(sw, pce0, swc_ww*SF0+swc_ow*(1-SF0), sor_ww*SF0+sor_ow*(1-SF0),
  teta=teta_ww*SF0+teta_ow*(1-SF0))
sw_b=fzero(f1, 0.5)
println(sw_b)
BCs.right.a[:]=0.0
BCs.right.b[:]=1.0
...
# dt=(W/Nx)/u_in/20 # [s]
pv_inj=5
t_end = pv_inj*V_core/(u_in*A_core) # [s] final time
dt=H/Nx/u_in
